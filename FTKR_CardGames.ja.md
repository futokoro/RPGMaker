...
以下のプラグインパラメータの値を任意に設定してください。
※基本的には変更不要です。

## 共通
#### Suit Type
使用するスートを、スペード(spade)、ハート(heart)、クラブ(club)、ダイア(diamond)の4種類から変える場合は設定を変更してください。

設定はスートの英字をカンマ(,)で区切って入力します。<br>
なお、スートは必ず偶数設定してください。奇数の場合は、ゲームが成り立ちません。

#### Max Rank
使用するランクを、１(A)～13(K)の13種類から変える場合は設定を変更してください。
