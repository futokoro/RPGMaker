...
#### クリティカル時のみ ダメージID1 のダメージを追加させる
通常、クリティカル時のダメージは「基本のダメージ×クリティカル倍率」ですが、以下のような設定にすることで、「基本のダメージ＋クリティカル時の追加ダメージ」のように加算させることもできます。

* 加算にする場合はダメージID0およびダメージID1のクリティカル倍率を100にする。ただし、100以外でも問題ありません。
* ダメージID1の有効条件を「クリティカルが発生した場合」にする。スクリプトは下記参照。
* ダメージID1のダメージ計算式で設定したダメージが、クリティカル時に追加されます。
...
スキルId: 10
クリティカル倍率: 100
有効条件: b.result().critical
</EIC ダメージID>
```

ダメージID1の参照スキル(上記の例ではスキルID10)のダメージ計算式は、お好きなように。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.1.2](FTKR_ExItemConfig_Damage.js) | 2020/01/13 | ダメージID1以降でダメージ倍率を0にした場合に、ダメージID0でのクリティカル判定の結果を問わず、会心メッセージを表示しない不具合を修正<br>ダメージIDの計算式に攻撃対象のパラメータも参照できるように修正 |
| ver1.1.1 | 2020/01/11 | プラグインパラメータの読み取り部の不具合により、プラグインが正常に動作していなかった箇所を修正 |
