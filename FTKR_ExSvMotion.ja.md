...
なお、パーティーコマンドをスキップするプラグインと併用する場合は、プラグインパラメータ`Enabled Refresh In PartyCmd`を有効に設定してください。
...
| action  | 行動モーション全般 | action を設定しない場合は、すべての行動モーションよりも優先が高くなります。 |
...
また、行動モーションの優先度を他の状態モーションよりも下げることで、例えば、防御中は行動モーション(ダメージや回避など)を取らないなどの設定ができます。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.4.0](FTKR_ExSvMotion.js) | 2019/04/13 | 状態モーションの優先度に、行動モーション action の状態を設定する機能を追加 |
| ver1.3.2 | 2019/03/10 | パーティーコマンドをスキップするプラグインに対応するプラグインパラメータ Enabled Refresh In PartyCmd を追加 |
| ver1.3.1 | 2018/05/31 | パーティーの誰かが防御を使用すると、パーティーメンバー全員が防御モーションを一瞬実行する不具合を修正 |
