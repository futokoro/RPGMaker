 ...
 * 
 * 1. アイテム合成画面の表示
 * ICS Open [compositTitle] [slotTitle] [statusTitle] [commandList]
 * ICS アイテム合成画面表示 [合成タイトル] [スロットタイトル] [情報タイトル] [コマンドリスト]
 *    :[]部の入力は必須ではありません。
...
(function() {
    ...
    FTKR.ICS.command = {
        ...
        list          :String(parameters['Command List'] || ''),
        format:{
            material  :String(parameters['Change Materials Name'] || ''),
            recipe    :String(parameters['Change Resipes Name'] || ''),
    ...
    FTKR.ICS.status = {
        ...
        unkouwn       :String(parameters['Unkouwn Item Name'] || ''),
        number        :String(parameters['Composit Number Format'] || ''),
    ...
    //確認ウィンドウ設定
    FTKR.ICS.confTitle = {
        format        :String(parameters['Conf Title Format'] || ''),
    };
    FTKR.ICS.conf = {
    ...
    //合成結果ウィンドウ設定
    FTKR.ICS.result = {
        format        :String(parameters['Result Title Format'] || ''),
        great         :String(parameters['Result Great Success'] || ''),
        success       :String(parameters['Result Success'] || ''),
        failure       :String(parameters['Result Failure'] || ''),
    ...
    FTKR.ICS.systemSe = {
        success:{
            name:String(parameters['Success SE Name'] || 'Sound2'),
            volume:Number(parameters['Success SE Volume'] || 0),
            pitch:Number(parameters['Success SE Pitch'] || 0),
            pan:Number(parameters['Success SE Pan'] || 0),
        },
        great:{
            name:String(parameters['Great SE Name'] || 'Sound2'),
            volume:Number(parameters['Great SE Volume'] || 0),
            pitch:Number(parameters['Great SE Pitch'] || 0),
            pan:Number(parameters['Great SE Pan'] || 0),
        },
        failure:{
            name:String(parameters['Failure SE Name'] || 'Sound2'),
            volume:Number(parameters['Failure SE Volume'] || 0),
            pitch:Number(parameters['Failure SE Pitch'] || 0),
            pan:Number(parameters['Failure SE Pan'] || 0),
        },
        lost:{
            name:String(parameters['Lost SE Name'] || 'Sound2'),
            volume:Number(parameters['Lost SE Volume'] || 0),
            pitch:Number(parameters['Lost SE Pitch'] || 0),
            pan:Number(parameters['Lost SE Pan'] || 0),
        },
    };
        ...
        for (var i = 0; i < datas.length; i++) {
            ...
            if (code) {
                ...
                switch (match[1].toUpperCase()) {
                    ...
                    case 'DIFFICULTY':
                    case '難易度':
                        recipe.setDifficulty((match[2].replace(/\s/g, '')).split(','));
                        break;
                    default:
    ...
    Window_IcsCompsiTitle.prototype.refresh = function() {
        this.contents.clear();
        var text = FTKR.ICS.windowText.title || FTKR.ICS.compositTitle.format;
    ...
    Window_IcsCommand.prototype.makeCommandList = function() {
        var commandList = FTKR.ICS.windowText.command || FTKR.ICS.command.list;
    ...
    Window_IcsCompsiSlotTitle.prototype.refresh = function() {
        this.contents.clear();
        var text = FTKR.ICS.windowText.slotTitle || FTKR.ICS.slotTitle.format
        this.drawTextEx(text, 0, 0);
    };
    ...
    Window_IcsCompsiStateTitle.prototype.refresh = function() {
        this.contents.clear();
        var text = FTKR.ICS.windowText.statusTitle || FTKR.ICS.statusTitle.format;
        this.drawTextEx(text, 0, 0);
    };
    ...
    Window_IcsCompsiState.prototype.drawDifficulty = function(value, x, y) {
        ...
        var text = FTKR.ICS.status.diffFormat.format(value);
    ...
    Window_IcsCompsiState.prototype.refresh = function() {
        ...
        if (comp) {
            ...
            if (has > 0 && number) {
                ...
                this.drawText(FTKR.ICS.result.success, 0, 0, w/2);
                ...
                var dy = 0;
                if (recipe.great()) {
                    this.drawText(FTKR.ICS.result.great, 0, y*(2 + dy), w/2);
                ...
                if (recipe.failure()) {
                    this.drawText(FTKR.ICS.result.failure, 0, y*(2 + dy), w/2);
