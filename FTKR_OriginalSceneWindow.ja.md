...
1. [オリジナルウィンドウの設定](#オリジナルウィンドウの設定)
    ...
    1. [セレクトウィンドウの設定パラメータ](#コマンドウィンドウの設定パラメータ)
...
バトルシーンの場合は、自動的にウィンドウの生成が行われます。
特に操作は行ないません。

### ウィンドウの生成数の設定
各ウィンドウを何個まで生成できるかは、シーン毎、ウィンドウタイプ毎に、プラグインパラメータを設定します。
ここで設定した生成数から 1 引いた数までのウィンドウIDを使用できるようになります。

#### オリジナルシーンでの生成数
`*** Number In Original`

*** はウィンドウタイプにより、Common、Command、Select と変わります。
0 を設定した場合は、ウィンドウを作成することはできません。

#### マップシーンでの生成数
`*** Number In Map`

仕様は、オリジナルシーンのパラメータと同じです。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.2.1](FTKR_OriginalSceneWindow.js) | 2017/06/23 | 表示内容の設定で、ステータスに制御文字を使用すると正しく反映されない不具合を修正<br>ウィンドウの表示をOFFにした時に、アクティブがOFFにならない不具合を修正<br>プラグインパラメータに@typeを対応 |
