...
* このコマンドは、どのシーンのウィンドウであっても、マップ上で実行してください。
...
HEIGHT [height]
```
ウィンドウの高さを pixel単位 または 行単位 で指定します。<br>
* heightに数字で指定すると、pixel単位の設定になります。
* heightに`数字＋L`で指定すると、行単位の設定になります。(例: 3L)

行単位の設定の場合、先に設定していた`行の高さ`と`余白`の設定値から高さを算出します。<br>
```
行単位の高さ = 設定した行数 × 行の高さ + 余白 × 2
```

height に -1 を入力すると、画面の高さに合わせます。<br>
コマンドウィンドウに対しても設定可能です。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.7.1](FTKR_OriginalSceneWindow.js) | 2020/03/21 | アクティブウィンドウ命令が正常に動作しない不具合を修正<br>ウィンドウの高さと幅を個別に設定する機能を追加<br>コマンドウィンドウの高さをコマンド数に寄らずに設定する機能を追加 |
| ver1.7.0 | 2019/12/16 | コモンウィンドウに対して、FTKR_CustomSimpleActorStatus.jsのstatusList方式で表示内容を設定する機能を追加 |
