...
## 立ち絵を使った表示
立ち絵のような高さがある画像を使った場合の設定例を示します。

![画像](image/FTKR_CustomSimpleActorStatus/n07_005.png)

### レイアウトの設定

画像にステータスを重ねて表示させる方法を使います。
立ち絵は`image`で表示します。
...
デフォルトの画面サイズで横に４人並べる場合は、`BGI WIDTH`を 128 にすると、ちょうど良いサイズになります。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.7.0](FTKR_CustomSimpleActorStatus.js) | 2017/06/02 | アクター毎に個別に設定できるカスタムゲージを追加<br>クラス毎に個別に設定できるカスタムゲージを追加<br>プラグインパラメータEnabled Skill Statusを無効にした場合にスキル画面のステータス欄が表示しない不具合を修正 |
