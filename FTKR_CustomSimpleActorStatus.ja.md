...
| [AOP装備パラメータ](#AOP装備パラメータの表示) | eaop(x) | × | 装備変更後のオリジナルパラメータの数値を表示します |
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver2.3.0](FTKR_CustomSimpleActorStatus.js) | 2017/07/23 | 表示コードの判定部の記述を見直し<br>括弧で数値を指定する表示コードに対して、数値ではなくスクリプトで指定できるように修正<br>FTKR_AddOriginalParametersで作成したオリジナルパラメータの装備パラメータを表示するコード「eaop(x)」を追加 |
