...
なお、各ウィンドウのレイアウト変更は、公式プラグインの`AltMenuScreen.js`を使用しています。
この場合、プラグインは以下の順番になるように登録してください。
```
AltMenuScreen.js
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.4.3](FTKR_CustomSimpleActorStatus.js) | 2017/05/06 | アクターを横に並べた時に、顔画像が正しく表示されない不具合を修正<br>縦のカーソル間隔を設定する機能を追加 |
