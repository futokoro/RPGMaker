...
(function() {
    ...
    DataManager.isDatabaseLoaded = function() {
        ...
        if (!_BAM_DatabaseLoaded) {
            this.itemEvaluteNotetags($dataSkills);
            _BAM_DatabaseLoaded = true;
        }
        return true;
    };

    DataManager.itemEvaluteNotetags = function(group) {
        for (var n = 1; n < group.length; n++) {
            var obj = group[n];
            obj.evaluteFormura = readObjectMeta(obj, ['BAM_優先度式', 'BAM_EVALUTE']);
        }
    };
    ...
    var _BAM_Game_Action_evaluteWithTarget = Game_Action.prototype.evaluateWithTarget;
    Game_Action.prototype.evaluateWithTarget = function(target) {
        if (this.item().evaluteFormura) {
            return this.bamEvaluteFormura(target);
        } else {
            return _BAM_Game_Action_evaluteWithTarget.call(this, target) || 0;
        }
    };

    Game_Action.prototype.bamEvaluteFormura = function(target) {
        FTKR.setGameData(this.subject(), target, this.item());
        return FTKR.evalFormula(this.item().evaluteFormura);
    };
