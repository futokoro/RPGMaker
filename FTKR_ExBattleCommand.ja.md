...
下の画面のように、コマンドの間に空欄を設けます。この空欄はカーソルで選択できずに、自動的に次の行に移ります。複数の空欄を空けることもできます。

![画像](image/FTKR_ExBattleCommand/n02_004.png)
...
* escape  - 「逃げる」コマンド
* blank - 空欄
    * 空欄コマンドは、必ず、空欄以外のコマンドの間に設定してください。
...
![画像](image/FTKR_ExBattleCommand/n02_005.png)
...
* type : コマンドの種類に合わせて、以下の英字を指定します。
    ...
    * blank : 空欄
* ext : 「スキル」コマンドの場合は、ここでスキルタイプIDを指定します。
...
* priority : コマンドの表示順番を決めるための表示優先度を数値で入力します。
    ...
    * データベース間のメモ欄の優先度は「アクター＞職業＞装備（装備スロットの上側が優先度高）＞ステート」（左側が優先度高）になります。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver2.2.0](FTKR_ExBattleCommand.js) | 2019/12/29 | パーティーコマンドのカスタムコマンドに、指定したコモンイベントを実行する機能を追加。 |
