...
1. [概要](#概要)
2. [基本設定](#基本設定)
3. [アクターと職業の設定](#アクターと職業の設定)
4. [装備とステートの設定](#装備とステートの設定)
1. [アイテムとスキルの設定](#アイテムとスキルの設定)
5. [エネミーの設定](#エネミーの設定)
...
アクターと職業のメモ欄に以下のタグを入力することで、パラメータの初期値やレベルアップでの成長度合を設定できます。

## 初期値を設定する

オリジナルパラメータを使用するためには、初期値の設定が必要です。

下記のタグをアクターか職業のメモ欄に設定することで、初期値(レベル1の時のパラメータ)を設定できます。

```
<AOP Param x Level 1 Value: z (Rand r)>
<AOP 'code' Level 1 Value: z (Rand r)>
```
パラメータID x (または'code')の初期値を z に設定します。<br>
()内を追記した場合、初期値 z に対して ±r のランダム値を付加します。

### 入力例
Param 0 の code を str とした場合
```
...
```
アクターのレベルが y の時のパラメータID x (または'code')の増加量を z に設定します。<br>
()内を追記した場合、増加量 z に対して ±r のランダム値を付加します。<br>
...
<AOP 'code' Grow: y>
```
このタグを設定した武器・防具を装備した状態でレベルが上がると、パラメータ x (または'code')を y 加算します。<br>
この加算値は、装備を外した場合でも残ります。
...
アイテムおよびスキルのメモ欄に以下のタグを入力することで、使用時にパラメータを強化させることができます。

## パラメータの最大値の増減

アイテムおよびスキルのメモ欄に以下のタグを入力することで、パラメータの最大値を増減できます。
...
このタグを設定したアイテムやスキルを使用すると、対象のパラメータ x (または'code')を y 加算します。<br>
...
このタグを設定したアイテムやスキルを使用すると、対象のパラメータ x (または'code')の現在値に y 加算します。<br>
y に負の数字を入れた場合、減算します。<br>
y に'%'を付けた場合、パラメータ x (または'code')の最大値の y% 分加算します。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.3.0](FTKR_AddOriginalParameters.js) | 2020/03/21 | アイテムやスキル使用時にパラメータを成長させる機能を追加 |
| ver1.2.1 | 2018/10/16 | オリジナルパラメータの入力ミス時のエラー表示機能を追加。(F12) |
