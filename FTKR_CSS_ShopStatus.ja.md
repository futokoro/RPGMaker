...
3. [CSS表示コードのショップ画面特殊仕様](#CSS表示コードのショップ画面特殊仕様)
4. [ウィンドウの設定](#ウィンドウの設定)
...
ショップ画面では、`value`部に`shop`と文字列を入力することで、カーソルで選択中の装備品と同じ個所の現在装備中の装備を表示できます。
なお、`shop`ではなく、スクリプトで`item.etypeId-1`と入力することでも同じ結果になります。<br>
ただし、スクリプト入力の場合処理が重くなります。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver2.3.0](FTKR_CSS_ShopStatus.js) | 2019/05/12 | FTKR_CustomSimpleActorStatus の v3.5.3 に対応<br>プラグインパラメータの初期値を変更 |
| ver2.2.2 | 2018/12/27 | 武器防具のアイテム用パラメータのコードが正しく反映されない不具合を修正 |
| ver2.2.1 | 2018/12/13 | プラグインパラメータ初期値変更 |
