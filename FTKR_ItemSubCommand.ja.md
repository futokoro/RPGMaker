...
このサブコマンドは、武器、防具以外のアイテムのを選択した場合はサブコマンドウィンドウ上に表示しません。
...
スクリプト形式の条件式を入力します(*2)。
設定しない場合は、表示しません。
プラグインパラメータとアイテムの個別設定が両方ある場合は両方の条件を満たした場合にのみ表示します。

### 有効条件：サブコマンドウィンドウを実行するための条件
プラグインパラメータ`<Custom* Enabled Condition>`とアイテム毎の個別条件(*1)を設定できます。
スクリプト形式の条件式を入力します(*2)。
設定しない場合は、実行できません。
プラグインパラメータとアイテムの個別設定が両方ある場合は両方の条件を満たした場合にのみ実行できます。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.5.0](FTKR_ItemSubCommand.js) | 2018/02/22 | アイテム画面から装備する機能を追加 |
