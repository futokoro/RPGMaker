...
この場合は、`iv[x]`の替わりに、`item._seflVariables._data[x]`と入力してください。
同様に、`av[x]`は`a._seflVariables._data[x]`、`bv[x]`は`b._seflVariables._data[x]`と入力してください。
...
* タイプには、アイテム(item)、武器(weapon)、防具(armor)、スキル(skill)、アクター(actor)、エネミー(enemy)を代入してください。
...
| バージョン | 公開日 | 更新内容 |
| --- | --- | --- |
| [ver1.1.0](FTKR_ItemSelfVariables.js) | 2017/04/18 | 個別にセルフ変数の数を設定できるように変更 <br> アクターとエネミーにセルフ変数を追加 |
